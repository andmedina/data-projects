...
def test_reference_and_encounter_normalization():
    encounter = {
        ...
        "id": "e1",
        "subject": {"reference": "https://example.org/fhir/Patient/p1"},
        "status": "finished",
    }
...
def test_observation_without_subject_is_rejected():
    assert "MISSING_OBSERVATION_SUBJECT" in validate_resource(
...
def test_observation_rejects_unsupported_value_type():
    observation = {
        ...
        "status": "final",
        "subject": {"reference": "Patient/p1"},
        "valueRange": {"low": {"value": 1}, "high": {"value": 2}},
    }
...
def test_lab_incident_fixtures_show_missing_then_corrected_value(tmp_path: Path):
    ...
    corrected_rows = [
        json.loads(line)["canonical"] for line in (corrected_dir / "accepted.jsonl").read_text().splitlines()
    ]
