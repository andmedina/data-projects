...
def test_generator_is_deterministic():
    ...
    lab_observations = [
        entry["resource"] for entry in bundle["entry"] if entry["resource"]["resourceType"] == "Observation"
    ]
    assert lab_observations
    assert all(observation["category"][0]["coding"][0]["code"] == "laboratory" for observation in lab_observations)
    coverages = [entry["resource"] for entry in bundle["entry"] if entry["resource"]["resourceType"] == "Coverage"]
    assert len(coverages) == 3
    assert all(coverage["period"]["start"] <= coverage["period"]["end"] for coverage in coverages)
    assert {coverage["payor"][0]["reference"] for coverage in coverages} == {"Organization/synthetic-payer-001"}
    imaging_studies = [
        entry["resource"] for entry in bundle["entry"] if entry["resource"]["resourceType"] == "ImagingStudy"
    ]
    assert all(study["numberOfSeries"] == len(study["series"]) for study in imaging_studies)
    assert all(
        study["numberOfInstances"] == sum(series["numberOfInstances"] for series in study["series"])
    ...
    canonical_rows = [
        {
            ...
            "payer_id": "payer-a",
            "status": "active",
            "coverage_start": "2025-01-15",
            "coverage_end": "2025-02-10",
        },
        {
            "resource_type": "Coverage",
            "source_resource_id": "cov-2",
            "patient_id": "p2",
            "payer_id": "payer-a",
            "status": "active",
